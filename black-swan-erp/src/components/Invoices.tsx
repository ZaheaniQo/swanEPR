...
import { TaxInvoice, InvoiceStatus, Role, InvoiceType } from '../types';
import { 
  FileText, CheckCircle, RefreshCw, ChevronDown, Lock, Eye, 
  Plus, Search, Filter, Calendar, ShieldCheck 
} from 'lucide-react';
...
const Invoices: React.FC = () => {
  ...
  const loadData = useCallback(async (reset = false) => {
    ...
    try {
      const cursor = reset ? undefined : (lastId || undefined);
      const result = await dataService.getTaxInvoices(20, cursor);
    ...
    <div className="space-y-6">
                ...
                <tbody className="divide-y divide-border">
                    {filteredInvoices.map(inv => (
                        <tr key={inv.id} className="hover:bg-secondary/30 cursor-pointer" onClick={() => navigate(`/invoices/${inv.id}`)}>
                            ...
                            <td className="p-4 text-text-muted">{inv.issueDate.split('T')[0]}</td>
                ...
                <Card key={inv.id} className="p-4" onClick={() => navigate(`/invoices/${inv.id}`)}>
                    <div className="flex justify-between items-start mb-2">
                        <div>
                            ...
                            <p className="text-xs text-text-muted">{inv.issueDate.split('T')[0]}</p>
                        </div>
        ...
        {hasMore && (
            <div className="flex justify-center pt-4">
                <Button variant="ghost" onClick={() => loadData(false)} disabled={loading}>
