...
const Accounting: React.FC = () => {
  ...
  const loadData = async () => {
    const lResult = await dataService.getLedgerEntries();
    // Map JournalEntry to LedgerEntry structure for list display
    const mappedLedger: LedgerEntry[] = lResult.items.map(je => ({
        id: je.id,
        date: je.date,
        documentNumber: je.reference || je.entryNumber,
        description: je.description,
        amount: je.totalDebit,
        type: 'DEBIT', // Simplified assumption for list view
        department: 'General'
    }));
    setLedger(mappedLedger);
    
    // Use proper TaxInvoice fetch
    const invResult = await dataService.getTaxInvoices();
