  ...
  useEffect(() => {
    ...
    const loadData = async () => {
        try {
                ...
                const msg = err?.message || 'تعذر تحميل بيانات الموظفين';
                showToast(msg, 'error');
                // إذا انتهت الجلسة نعيد تسجيل الدخول بدلاً من الخروج الصامت
  ...
  return (
    <div className="space-y-6">
        ...
        {showLeaveModal && selectedEmp && (
            <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowLeaveModal(false)}>
        ...
        {/* Hiring modal */}
        {showHiringModal && (
            <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowHiringModal(false)}>
                <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
