...
import { ArrowLeftRight, Layers, Search, Shirt } from 'lucide-react';
...
const Inventory: React.FC = () => {
  ...
  const [searchTerm, setSearchTerm] = useState('');
    ...
    useEffect(() => {
        ...
        erpService.getInventoryStock().then((stock: RawStock[]) => {
            const mappedItems: InventoryItem[] = stock.map((s) => ({
                id: s.id,
                code: s.productCode || '',
                name: s.productName || '',
                type: s.type === 'MATERIAL' ? 'Material' : 'Product',
                quantity: s.quantity,
                unit: s.unit || 'PCS',
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-xl border border-border shadow-sm">
          ...
          <div className="flex gap-2">
                  ...
                  <button 
                    key={type}
                                        onClick={() => setFilterType(type as 'All' | 'Material' | 'Product')}
