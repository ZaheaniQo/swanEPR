  ...
  const [expandedContractId, setExpandedContractId] = useState<string | null>(null);
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : 'تعذر تحميل العقود، الرجاء تسجيل الدخول أو المحاولة لاحقاً';
              ...
              <div className="max-w-3xl mx-auto py-6">
                {currentStep === 1 && (
                    <div className="space-y-8 animate-slide-in">
                        <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                            <h3 className="font-bold text-lg text-slate-800 mb-4 flex items-center gap-2"><Building size={20} className="text-teal-600"/> الطرف الأول (المزود)</h3>
                             <div className="grid grid-cols-2 gap-4">
                                 <div className="col-span-2">
                                    <label className="label" htmlFor="partyA-legalName">الاسم القانوني للشركة</label>
                                    <input id="partyA-legalName" className="input-field" placeholder="اسم المصنع / الشركة" value={draftContract.partyA?.legalName} onChange={e => setDraftContract({...draftContract, partyA: {...draftContract.partyA!, legalName: e.target.value}})} />
                                 </div>
                                 <div>
                                    <label className="label" htmlFor="partyA-representative">اسم الممثل</label>
                                    <input id="partyA-representative" className="input-field" placeholder="الممثل النظامي" value={draftContract.partyA?.representativeName} onChange={e => setDraftContract({...draftContract, partyA: {...draftContract.partyA!, representativeName: e.target.value}})} />
                                 </div>
                                 <div>
                                    <label className="label" htmlFor="partyA-email">البريد الإلكتروني (للإشعارات)</label>
                                    <div className="relative">
                                        <Mail size={16} className="absolute top-3 right-3 text-slate-400"/>
                                        <input id="partyA-email" className="input-field pr-10" placeholder="email@company.com" value={draftContract.partyA?.email} onChange={e => setDraftContract({...draftContract, partyA: {...draftContract.partyA!, email: e.target.value}})} />
                                    </div>
                                 </div>
                                 <div className="col-span-2">
                                     <label className="label" htmlFor="partyA-address">العنوان الوطني</label>
                                     <input id="partyA-address" className="input-field" placeholder="العنوان كاملاً" value={draftContract.partyA?.address} onChange={e => setDraftContract({...draftContract, partyA: {...draftContract.partyA!, address: e.target.value}})} />
                                 </div>
                             </div>
                        </div>
                         <div className="bg-indigo-50 p-6 rounded-xl border border-indigo-200">
                             ...
                             {/* Auto-Fill Dropdown */}
                             <div className="mb-4">
                                 <label className="label" htmlFor="partyB-existing">اختر عميل حالي (تعبئة تلقائية)</label>
                                 <select id="partyB-existing" className="input-field" value={selectedCustomerId} onChange={handleCustomerSelect}>
                             ...
                             <div className="grid grid-cols-2 gap-4 border-t border-indigo-200 pt-4">
                                 <div className="col-span-2">
                                    <label className="label" htmlFor="partyB-legalName">الاسم القانوني / العميل</label>
                                    <input id="partyB-legalName" className="input-field" placeholder="اسم الشركة أو العميل" value={draftContract.partyB?.legalName} onChange={e => setDraftContract({...draftContract, partyB: {...draftContract.partyB!, legalName: e.target.value}})} />
                                 </div>
                                 <div>
                                    <label className="label" htmlFor="partyB-representative">اسم الممثل</label>
                                    <input id="partyB-representative" className="input-field" placeholder="الممثل" value={draftContract.partyB?.representativeName} onChange={e => setDraftContract({...draftContract, partyB: {...draftContract.partyB!, representativeName: e.target.value}})} />
                                 </div>
                                 <div>
                                    <label className="label" htmlFor="partyB-email">البريد الإلكتروني (لإرسال العقد)</label>
                                    <div className="relative">
                                        <Mail size={16} className="absolute top-3 right-3 text-slate-400"/>
                                        <input id="partyB-email" className="input-field pr-10" placeholder="client@email.com" value={draftContract.partyB?.email} onChange={e => setDraftContract({...draftContract, partyB: {...draftContract.partyB!, email: e.target.value}})} />
                                    </div>
                                 </div>
                                 <div className="col-span-2">
                                     <label className="label" htmlFor="partyB-address">العنوان</label>
                                     <input id="partyB-address" className="input-field" placeholder="عنوان العميل" value={draftContract.partyB?.address} onChange={e => setDraftContract({...draftContract, partyB: {...draftContract.partyB!, address: e.target.value}})} />
                                 </div>
                             </div>
                        </div>
                    </div>
                ...
                {currentStep === 2 && (
                    <div className="space-y-6 animate-slide-in">
                        <div>
                             <label className="label" htmlFor="contract-title">عنوان العقد / المشروع</label>
                             <input id="contract-title" className="input-field w-full text-lg font-bold" placeholder="مثال: توريد زي موحد شتوي 2024" value={draftContract.title} onChange={e => setDraftContract({...draftContract, title: e.target.value})} />
                        </div>
                        <div className="grid grid-cols-2 gap-6">
                            <div>
                                <label className="label" htmlFor="contract-total-value">إجمالي قيمة العقد</label>
                                <div className="relative">
                                    <DollarSign className="absolute right-3 top-3 text-slate-400" size={18} />
                                    <input id="contract-total-value" type="number" className="input-field w-full pr-10" value={draftContract.totalValue} onChange={e => setDraftContract({...draftContract, totalValue: Number(e.target.value)})} />
                                </div>
                            </div>
                            <div>
                                <label className="label" htmlFor="contract-currency">العملة</label>
                                <select id="contract-currency" className="input-field w-full" value={draftContract.currency} onChange={e => setDraftContract({...draftContract, currency: e.target.value})}>
                         ...
                         <div>
                             <label className="label" htmlFor="contract-description">وصف النطاق</label>
                             <textarea id="contract-description" rows={4} className="input-field w-full" placeholder="وصف تفصيلي للأعمال أو المنتجات..." />
                        </div>
                    </div>
                )}
                ...
                {currentStep === 3 && (
                    <div className="space-y-6 animate-slide-in">
                        ...
                        {draftContract.paymentTerms?.map((term) => {
                            const descId = `payment-desc-${term.id}`;
                            const amountTypeId = `payment-type-${term.id}`;
                            const amountValueId = `payment-value-${term.id}`;
                            const triggerId = `payment-trigger-${term.id}`;
                            return (
                                <div key={term.id} className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-12 gap-4 items-end">
                                    <div className="col-span-4">
                                        <label className="label" htmlFor={descId}>وصف الدفعة</label>
                                        <input id={descId} className="input-field" value={term.name} onChange={e => updatePaymentTerm(term.id, { name: e.target.value })} />
                                    </div>
                                    <div className="col-span-3">
                                        <label className="label" htmlFor={amountTypeId}>القيمة / النسبة</label>
                                    ...
                                    <div className="col-span-4">
                                        <label className="label" htmlFor={triggerId}>تستحق عند</label>
                                        <select 
                                            id={triggerId}
                                            className="input-field"
                                            value={term.trigger}
                                    ...
                                    <div
                                        ...
                                        onKeyDown={(event) => {
                                            if (event.key === 'Enter' || event.key === ' ') {
                                                ...
                                                setExpandedContractId(expandedContractId === contract.id ? null : contract.id);
                                            }
                                        }}
                                    >
                                                ...
                                                <table className="w-full text-sm text-left bg-white border border-slate-200 rounded-lg overflow-hidden">
                                                    <thead className="bg-slate-100 text-slate-500">
                                                        <tr>
                                                            ...
                                                            <th className="p-3 text-center">{t('col.actionsColumn')}</th>
                                                        </tr>
                                                    </thead>
                                                ...
                                                <table className="w-full text-sm text-left bg-white border border-slate-200 rounded-lg overflow-hidden">
                                                    ...
                                                    <tbody className="divide-y divide-slate-100">
                                                        ...
                                                        {contractExpenses.length === 0 && (
                                                            <tr><td colSpan={4} className="p-4 text-center text-slate-400 text-xs">{t('contracts.expenses.empty')}</td></tr>
                                                        )}
                                                    </tbody>
                                                </table>
                                            </div>
