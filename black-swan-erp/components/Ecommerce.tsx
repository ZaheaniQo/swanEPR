...
import { EcommerceProduct, CartItem } from '../types';

const products: EcommerceProduct[] = [
    { id: 1, name: 'Premium Surgeon Gown', price: 45, image: 'https://images.unsplash.com/photo-1584036561566-b45238f2e121?auto=format&fit=crop&q=80&w=400', rating: 4.8 },
    { id: 2, name: 'Nurse Scrub Set (Teal)', price: 30, image: 'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&q=80&w=400', rating: 4.5 },
    { id: 3, name: 'Lab Coat (Anti-Microbial)', price: 55, image: 'https://images.unsplash.com/photo-1579684385127-1ef15d508118?auto=format&fit=crop&q=80&w=400', rating: 4.9 },
    { id: 4, name: 'Medical Crocs', price: 25, image: 'https://images.unsplash.com/photo-1603400521630-9f2de124b33b?auto=format&fit=crop&q=80&w=400', rating: 4.2 },
    { id: 5, name: 'Surgical Cap', price: 12, image: 'https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?auto=format&fit=crop&q=80&w=400', rating: 4.7 },
    { id: 6, name: 'Patient Gown', price: 18, image: 'https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?auto=format&fit=crop&q=80&w=400', rating: 4.0 },
];
    ...
    <div className="space-y-6 relative min-h-screen">
      ...
      <div className="bg-gradient-to-r from-teal-600 to-emerald-600 rounded-3xl p-8 text-white flex flex-col md:flex-row items-center justify-between shadow-2xl shadow-teal-900/20 overflow-hidden relative">
        ...
        <div className="space-y-4 max-w-lg relative z-10">
            <h2 className="text-3xl font-bold tracking-tight">{t('ecommerce.heroTitle')}</h2>
            <p className="text-teal-100 text-lg">{t('ecommerce.heroText')}</p>
        ...
        {products.map((product) => (
            <div key={product.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl transition-all duration-300 group">
                <div className="relative h-64 overflow-hidden">
                    <img src={product.image} alt={product.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 relative z-10" />
          ...
          <div className={`absolute top-0 bottom-0 ${lang === 'ar' ? 'left-0' : 'right-0'} w-full max-w-md bg-white shadow-2xl transition-transform duration-300 transform ${isCartOpen ? 'translate-x-0' : (lang === 'ar' ? '-translate-x-full' : 'translate-x-full')} flex flex-col`}>
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                  <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                      <ShoppingCart className="text-teal-600" /> {t('ecommerce.cart')}
                  </h2>
                      ...
                      cart.map(item => (
                          <div key={item.id} className="flex gap-4 p-4 bg-white border border-slate-100 rounded-xl shadow-sm">
                              ...
                              <div className="flex-1">
                                  ...
                                  <p className="text-teal-600 font-bold mb-3">${item.price * item.quantity}</p>
              ...
              {cart.length > 0 && (
                  <div className="p-6 bg-slate-50 border-t border-slate-200">
                      <div className="flex justify-between items-center mb-6">
                          <span className="text-slate-500 font-medium">{t('ecommerce.total')}</span>
                      ...
                      <div className="space-y-3">
                          ...
                          >
                              <User size={18} /> {t('ecommerce.checkoutB2C')}
                          </button>
                          ...
                          >
                              <Building2 size={18} /> {t('ecommerce.checkoutB2B')}
                          </button>
                      </div>
                  </div>
              )}
