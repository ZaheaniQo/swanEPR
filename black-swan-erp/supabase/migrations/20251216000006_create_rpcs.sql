-- Recreate RPCs with tenant_id support and new columns
...
BEGIN
  INSERT INTO employees (
    ...
    salary, join_date, status, iban, national_id, contract_type, tenant_id
  ) VALUES (
...
BEGIN
  ...
  IF jsonb_array_length(p_milestones) > 0 THEN
    INSERT INTO contract_milestones (contract_id, title, amount, percentage, due_date, status, paid_at, tenant_id)
    SELECT 
      v_contract_id,
      x->>'title',
      (x->>'amount')::numeric,
...
BEGIN
  ...
  WHERE product_id = p_product_id AND warehouse_id = p_warehouse_id AND tenant_id = p_tenant_id;
  ...
  INSERT INTO inventory_movements (
    ...
    p_product_id, p_warehouse_id, p_type, ABS(p_quantity), now(), p_user_id, p_tenant_id
  );
END;
$$ LANGUAGE plpgsql SECURITY INVOKER;
...
BEGIN
  INSERT INTO quotations (
    ...
    subtotal, vat_amount, total_amount, notes, customer_details, tenant_id
  ) VALUES (
  ...
  IF jsonb_array_length(p_items) > 0 THEN
    INSERT INTO quotation_items (quotation_id, description, quantity, unit_price, vat_rate, tenant_id)
    SELECT 
      v_quotation_id,
      x->>'description',
      (x->>'quantity')::numeric,
...
BEGIN
  ...
  LOOP
    -- Dynamic Account Lookup by Code if ID is not provided
    IF v_line->>'account_id' IS NOT NULL THEN
       v_account_id := (v_line->>'account_id')::uuid;
    ELSE
       SELECT id INTO v_account_id FROM coa_accounts WHERE code = v_line->>'account_code' AND tenant_id = p_tenant_id;
       IF v_account_id IS NULL THEN
         RAISE EXCEPTION 'Account code % not found for tenant', v_line->>'account_code';
       END IF;
    END IF;

    INSERT INTO journal_lines (
      journal_id, account_id, cost_center_id, description, debit, credit, tenant_id
    ) VALUES (
      v_journal_id,
      v_account_id,
...
BEGIN
  ...
  INSERT INTO receipts (
    ...
    customer_name, amount, date, payment_method, notes, tenant_id
  ) VALUES (
    p_receipt_number, p_contract_id, p_contract_title, p_milestone_id,
    p_client_name, p_amount, now(), p_method, p_notes, p_tenant_id
  );
  ...
  -- Lookup Accounts
  SELECT id INTO v_cash_account_id FROM coa_accounts WHERE code = '1001' AND tenant_id = p_tenant_id; -- Cash
  SELECT id INTO v_ar_account_id FROM coa_accounts WHERE code = '1100' AND tenant_id = p_tenant_id; -- AR
  ...
  INSERT INTO journal_entries (
    ...
    'POSTED', 'system', p_tenant_id
  ) RETURNING id INTO v_journal_id;

  INSERT INTO journal_lines (journal_id, account_id, description, debit, credit, tenant_id)
  VALUES 
    (v_journal_id, v_cash_account_id, 'Cash Receipt', p_amount, 0, p_tenant_id),
    (v_journal_id, v_ar_account_id, 'AR Clearing', 0, p_amount, p_tenant_id);
