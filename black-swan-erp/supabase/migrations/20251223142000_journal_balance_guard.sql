-- Allow controlled bypass of journal balance trigger during atomic postings
...
declare
  ...
  v_skip text;
begin
  v_skip := current_setting('app.skip_journal_balance', true);
  if v_skip = 'on' then
...
begin
  ...
  if v_total > 0 then
    ...
    perform set_config('app.skip_journal_balance', 'on', true);
    ...
    perform set_config('app.skip_journal_balance', 'off', true);
